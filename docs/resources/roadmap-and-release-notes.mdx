...
- [Custom reporting tags](/docs/learning-how-tos/examples-and-guides/how-to-use-custom-reporting-tags) now work for the [Voice API](/docs/voice/overview) in addition to text translation. Send the `X-DeepL-Reporting-Tag` header on the [session request](/api-reference/voice/request-session) and the whole session's usage is attributed to that tag.
