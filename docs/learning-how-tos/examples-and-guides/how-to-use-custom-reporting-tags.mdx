...
description: "Learn how to attach reporting tags to translate and Voice API requests so you can break down usage by team, project, or category."
public: true
---

**This guide shows you:**
- When to use custom reporting tags
- How to attach a tag to a translate or Voice API request via the `X-DeepL-Reporting-Tag` header
...
<Warning>
Custom tag data is recorded for text translation and Voice API requests. Support for additional request types will be added in a future update.
</Warning>
...
The WebSocket connection itself carries no headers, so the session request is the only place a tag can be set. You cannot change or add a tag once a session is running, and you cannot tag individual utterances within a session — to attribute audio to more than one tag, open a separate session per tag.
