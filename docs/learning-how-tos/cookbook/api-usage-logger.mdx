...
The project is intended for teams that need more flexibility than DeepL's built-in reporting tools provide. If you want account-wide views via a single API call instead, see the [Usage Analytics Dashboard](/docs/learning-how-tos/cookbook/usage-analytics-dashboard) cookbook, which uses the [Admin API](/api-reference/admin-api/organization-usage-analytics).
...
- **Streamlit dashboard** with three views: a Usage breakdown table, an Errors breakdown table, and a SQL Explorer for arbitrary DuckDB queries against the underlying table
